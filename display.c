/*
*  TFT Display Handling Example for AVR Atmega32
*  
...
static void num2arr( unsigned int num,  char *bufr ) {
  itoa(num, bufr, 16);
}
//*****************************************************
...
while(1) {           
  ...
  if(!busy)  
      ...
      { 
		...
		if ( uart_pkt_rdy == 1)  {
                   uart_pkt_rdy = 0;     // new uart packet has just arrived
                }
#endif
                        } 
                    } 
           ...
           /*  secondary tick   */
	 if (TCNT2 >= 250)  {
   /* do some jobs here if feel like it */ 
                        }
